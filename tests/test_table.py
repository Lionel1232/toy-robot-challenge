...
def test_valid_position_returns_false_for_invalid_position(x, y, mocker):
    table = Table(5, 5)
    position = Position(x, y, mocker.Mock())

    assert table.valid_position(position) is False


@pytest.mark.parametrize("x, y", [(0, 0), (4, 4), (3, 3)])
def test_valid_position_returns_true_for_valid_position(x, y, mocker):
    table = Table(5, 5)
    position = Position(x, y, mocker.Mock())
